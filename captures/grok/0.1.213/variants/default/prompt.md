...
Update the goal progress dashboard. Call this tool whenever the goal state changes. Examples:

After planning deliverables:
  update_goal(phase: "planning", deliverables: [{id: 1, title: "Setup project", status: "pending"}, {id: 2, title: "Core logic", status: "pending"}])

When starting work on a deliverable:
  update_goal(deliverables: [{id: 1, title: "Setup project", status: "in_progress"}])

When a deliverable is done:
  update_goal(phase: "executing", deliverables: [{id: 1, title: "Setup project", status: "verified"}])

When the entire goal is complete:
  update_goal(completed: true, message: "All deliverables verified")

Call this after EVERY meaningful step — the user sees a live dashboard powered by it.
...
```json
{
  ...
  "properties": {
    "phase": {
      "description": "Set the current phase. Use \"planning\" while defining deliverables, \"executing\" while workers run, \"verifying\" during verification.",
      "type": [
        "string",
        "null"
      ],
    ...
    "deliverables": {
      "description": "List of deliverables with their current status. Include ALL deliverables each time (not just changed ones). Each needs id (number), title (string), and status (\"pending\", \"in_progress\", or \"verified\").",
      "type": [
        "array",
        "null"
      ],
      "items": {
        ...
        "properties": {
          "id": {
            "description": "Deliverable ID (1-based index)",
            "type": "integer",
            "format": "uint32",
            "minimum": 0
          },
          ...
          "status": {
            "description": "Status: pending, in_progress, verified, failed",
            "type": "string"
          }
        },
        "required": [
    ...
    "completed": {
      "description": "Set to true ONLY when all deliverables are verified and the goal is fully achieved. This ends goal mode. Use together with `message` to include a completion summary.",
      "type": [
        "boolean",
        "null"
      ],
    ...
    "message": {
      "description": "Optional message surfaced to the user. Use with `completed: true` for a completion summary (e.g. \"All 4 deliverables verified, tests pass\"). Use standalone for a status-bar update (e.g. \"Running tests...\"). Do NOT put completion/success text in `blocked_reason` — use this field instead.",
      "type": [
        "string",
        "null"
      ],
    ...
    "blocked_reason": {
      "description": "ERROR ONLY. If set, the goal is paused as truly blocked: not achievable in this environment regardless of further worker effort (e.g. wrong target platform, missing credentials, contradictory spec). This is a FAILURE signal — never put success or achievement text here. For success, use `completed: true` with `message`.",
      "type": [
        "string",
        "null"
      ],
