...
```json
{
  ...
  "properties": {
    ...
    "head_limit": {
      "description": "Limit output to first N lines/entries, equivalent to \"| head -N\". When unspecified, shows all ripgrep results.",
      "type": "integer"
    },
    "multiline": {
...
- You can run agents in the background via background. Background agents return immediately; retrieve their results with get_command_or_subagent_output (call it in blocking mode to pause and wait when you need the results to continue).
