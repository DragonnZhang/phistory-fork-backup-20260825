...
- Use timeout_ms to limit wait time when blocking (default 30s, capped at ~10 min). You are notified when the task finishes, so you do not need to wait long.
...
```json
{
  ...
  "properties": {
    ...
    "timeout": {
      "description": "Optional timeout in milliseconds (max 36000000). Default: 120000 (2 minutes). `timeout: 0` in background mode disables the wrapper timeout entirely; the task runs until it exits or is killed via the kill task tool.",
      "type": [
        "integer",
        "null"
      ],
