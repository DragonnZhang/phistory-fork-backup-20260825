...
- Use timeout_ms to limit wait time when waiting for the task to complete (default 30s, capped at ~10 min). You are notified when the task finishes, so you do not need to wait long.
...
```json
{
  ...
  "properties": {
    ...
    "aspect_ratio": {
      "description": "Aspect ratio of the generated image, decide it based on the user's request. 1:1 for square (icons, profiles), 16:9 for wide (landscapes, cinematic), 9:16 for tall (phone wallpapers, stories), 3:2 for horizontal photos, 2:3 for vertical (portraits, posters).",
      "type": "string",
      "default": "auto"
    }
  },
...
Generate a video from a single source image and return the result so the model can continue with further actions. Provide `image` for the image to animate and optionally a `prompt` to guide the animation. Use this tool when the user provides an image and wants it animated, turned into a video, or used as the first frame. The source image may be a filesystem path, HTTPS URL, or `data:image/...;base64,...` URL. If `prompt` is omitted, a natural animation applies automatically. Duration can be 6 or 10 seconds; default to 6 unless the user requests longer. Resolution can be 480p or 720p; default to 480p unless the user specifically requests higher quality. Example: image_to_video(image="/Users/me/photo.jpg", prompt="gentle camera push-in with wind moving the hair", duration=6, resolution_name="480p")
...
Usage notes:
- Pass the task_id of the background task, monitor, or subagent you want to stop (a monitor's task_id is returned by monitor).
...
- Use when a background task, monitor, or subagent is stuck or no longer needed.
...
Generate a video from multiple reference images guided by a text prompt and return the result so the model can continue with further actions. Provide `images` with two or more image references and a required `prompt` describing the desired video. Use this tool when the user wants a video using multiple images as style/content references. Each reference may be a filesystem path, HTTPS URL, or `data:image/...;base64,...` URL. Choose `aspect_ratio` based on the user's request: 1:1 for square, 16:9 for wide/landscape/cinematic, 9:16 for tall/mobile, 3:2 for horizontal photos, 2:3 for vertical portraits/posters. Duration can be 6 or 10 seconds; default to 6. Resolution can be 480p or 720p; default to 480p. Example: reference_to_video(prompt="blend these into a cinematic fashion shot with slow dolly movement", images=["/Users/me/ref1.jpg", "/Users/me/ref2.jpg"], aspect_ratio="16:9", duration=6, resolution_name="480p")
...
```json
{
  ...
  "properties": {
    ...
    "images": {
      "description": "Reference images. Provide at least 2 entries; the images are used as style/content references for the generated video. Each entry may be an absolute filesystem path, HTTPS URL, or `data:image/...;base64,...` URL.",
      "type": "array",
      "items": {
        "type": "string"
      }
...
Use the spawn_subagent tool when at least one of these conditions is met. Note that these conditions are non-exhaustive:

1. **The user or invoked skill** requests delegation, sub-agents, orchestration, or parallel work (follow skill instructions when they prescribe spawning).
2. **A subagent_type is a clear match** for the work (see each one's description below).
3. **You can gain parallelism** by backgrounding subagent(s) while you continue meaningful main-thread work, and/or launch multiple independent subagents in one turn.

### Usage Notes

#### Prompting

Subagents start with no parent conversation history, so write as if you were briefing a smart colleague with no context. Your explanation should include:

1. What is being accomplish and why
2. What has already been learned or ruled out
3. Conventions to maintain

When resume_from is set, send only what changed since the last run (e.g. new feedback).

#### Resumption

Completed spawn_subagent tool calls return a `subagent_id`. You can pass this id to resume_from to continue a previously completed subagent's conversation.

Prefer resuming subagents when follow-up work related to work a previous agent already did since resumed subagents keep their prefix and tool state.

Note that a resumed agent must use the same subagent_type as the source.

### Agent/Subagent Types

#### Built-in

- **general-purpose**: Use for executing multi-step tasks including general implementation work. Has access to all tools: run_terminal_command, read_file, search_replace, list_dir, grep, web_search, todo_write (full set).
- **explore**: Use for read-only codebase search and file discovery. Has read-only tools: run_terminal_command, read_file, list_dir, grep.
- **plan**: Use for designing plans and/or implementation strategies. Has planning necessary tools: run_terminal_command, read_file, list_dir, grep, web_search, todo_write.
...
```json
{
  ...
  "properties": {
    "prompt": {
      "description": "The subagent's prompt.",
      "type": "string"
    },
    "description": {
      "description": "A short description (about 3-5 words) shown to the user, summarizing the agent's work.",
      "type": "string"
    },
    "subagent_type": {
      "description": "Subagent type to launch.",
      "type": "string",
      "default": "general-purpose"
    },
    "background": {
      "description": "If set true, subagent will launch in the background and this tool will immediately return a `subagent_id`. Use the task output tool to retrieve results.",
      "type": "boolean",
      "default": false
    },
    "capability_mode": {
      "description": "Capability mode: \"read-only\", \"read-write\", \"execute\", or \"all\". Controls which tools the child can use. Defaults are determind by `subagent_type`.",
      "type": [
        "string",
        "null"
      ],
    ...
    "isolation": {
      "description": "Isolation mode: \"none\" (default, shared workspace) or \"worktree\" (isolated git worktree). Worktree mode prevents the child's edits from affecting the parent workspace.",
      "type": [
        "string",
        "null"
      ],
    ...
    "resume_from": {
      "description": "Resume a previously completed subagent instead of starting fresh. Accepts a `subagent_id` returned by a prior call of this tool.",
      "type": [
        "string",
        "null"
      ]
    ...
    "cwd": {
      "description": "Working directory for the subagent. Mutually exclusive with isolation=\"worktree\" and ignored when resume_from is set (the resumed child inherits its source's cwd/worktree).",
      "type": [
        "string",
        "null"
      ]
...
Status update while working:
  update_goal(message: "Running tests...")

When the goal is fully achieved:
  update_goal(completed: true, message: "All tests pass, feature implemented")

When truly stuck after multiple failed attempts:
  update_goal(blocked_reason: "Cannot install dependency — missing system library")
...
Usage notes:
- task_ids: list of task IDs to wait for (from background=true commands or background=true subagents)
- mode: 'wait_any' returns when the first task completes, 'wait_all' waits for all tasks
- timeout_ms: optional max wait time in milliseconds (default 30s, capped at ~10 min). You are notified when tasks finish, so you do not need to wait long.
...
```json
{
  ...
  "description": "Input for the `wait_tasks` tool — blocks until multiple background tasks /\nsub-agents reach a terminal state.",
  "type": "object",
  "properties": {
    "task_ids": {
