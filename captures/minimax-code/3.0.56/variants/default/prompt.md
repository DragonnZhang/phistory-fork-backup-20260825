...
Ask the local desktop user one or more structured questions and pause this turn until the user replies in the UI. When a workflow requires user input or takeover, make an actual `ask_user` tool call; do not write `<ask_user>`, `ask_user(...)`, or a placeholder in plain prose because text does not pause the turn. For a final-action confirmation, the card itself must repeat the exact action, site, account, content or recipients, file names, visibility, and other material settings; do not rely on assistant prose outside the card. Use this exact shape: { "title": "Confirm", "steps": [{ "question": "Proceed?", "description": "Exact context", "options": [{ "label": "Proceed", "description": "Perform the exact action" }, { "label": "Cancel", "description": "Leave state unchanged" }], "selectionMode": "single" }] }. `question` is a string and `options` is an array; never nest question fields inside `question` or wrap the options array in another object. Keep questions concise and actionable.
...
```json
{
  ...
  "properties": {
    "status": {
      "description": "Required. Set to `complete` only when the objective is achieved and no required work remains. An explicit safety or policy refusal may be set to `blocked` immediately and does not require the three-turn threshold. For every other blocker, set to `blocked` only after the same blocking condition has recurred for at least three consecutive goal turns and the agent is at an impasse. After a previously blocked goal is resumed, a safety/policy refusal remains immediate; every other blocker starts a fresh blocked audit.",
      "anyOf": [
        {
          "const": "complete",
          "type": "string"
