...
1. The user explicitly asks ("what's the status", "how's it going", etc.).
...
<agent-context>
  ...
  YOUR WORKSPACE DIRECTORY: $PHISTORY_HOME/.minimax-code/sessions/mvs_214a53859ea04496a03379d4a6678c29/workspace
...
If your work produced, modified, or deleted a file deliverable (document, report, image,
...
- Local file deliverables: wrap them in <deliver-assets>...</deliver-assets>, each
  ...
  Example:
    ...
    <media src="/absolute/path/to/image.png" />
    <media type="file" src="/absolute/path/to/output.zip" caption="Generated archive" />
    <media src="/absolute/path/to/deleted.txt" deleted="true" caption="Deleted file" />
    </deliver-assets>
  Use absolute paths only. Include only files you actually created, modified, or deleted in this turn.
  Use deleted="true" only when the file existed before this turn and is now absent; never use it for
  a planned, missing, or failed output. For every non-deleted local file, verify it exists on disk
  using the current platform's shell
...
Launch one fresh child Agent for one concrete, bounded subtask.
...
```json
{
  ...
  "properties": {
    "description": {
      "minLength": 1,
      "description": "Short 3-5 word task label used for task tracking and the child title.",
      "type": "string"
    },
    "prompt": {
...
Read a local background task's output by task_id. Within this local runtime, omitting offset continues from the last successful read that omitted offset for this task in this session; pass offset=0 to replay or another explicit offset to choose a position. Explicit offset reads do not update that automatic position. The returned next_offset can also be passed explicitly. wait_ms optionally waits briefly for new output or a terminal status without stopping the task.
