...
<system-reminder>
Today: 2026-09-23; current working directory: '$PHISTORY_WORKSPACE'. Do not repeat this information in your reply.
</system-reminder>
...
<examples>
### Install distributions without replaying a failed cell
<example>
eval(language="py", code="%pip install pillow", title="install image support")
</example>
### Load an existing script; reuse its definitions in later cells
<example>
eval(language="py", code="%load ./analysis.py", title="load analysis")
</example>
### Install a JavaScript dependency outside the project
<example>
eval(language="js", code="%bun add csv-parse", title="install CSV parser")
</example>
### Execute an existing TypeScript script in the retained kernel
<example>
eval(language="js", code="%load ./analysis.ts", title="load analysis")
</example>
