...
Today is 2026-07-10, and the current working directory is '$PHISTORY_WORKSPACE'.
...
### Inputs
- `agent` (optional): The base agent type to use (e.g., `scout`, `reviewer`). Defaults to `task` (the general-purpose worker) — omit it for the default worker instead of passing `agent: "task"`.
