...
x-anthropic-billing-header: cc_version=2.1.243.083; cc_entrypoint=cli;
...
```json
{
  ...
  "properties": {
    ...
    "collection": {
      "description": "Database collection path: 1-15 \"/\"-separated segments (letters, digits, _ - . ~ : @ + per segment), so subcollections nest like \"boards/b1/columns\". Required for read_db and write_db.",
      "type": "string",
      "maxLength": 1000,
      "pattern": "^(?!\\.\\.?(?:\\/|$))[A-Za-z0-9_\\-.~:@+]{1,200}(?:\\/(?!\\.\\.?(?:\\/|$))[A-Za-z0-9_\\-.~:@+]{1,200}){0,14}$"
    },
