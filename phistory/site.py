...
function blocksSectionHtml(title, blocks, open) {
  ...
  const body = blocks.map(block => `<div class="prompt-block">${blockLabelHtml(block.label)}<div class="trace-rendered">${markdownHtml(block.text)}${originalTextHtml(block.text, block.originalText)}</div><pre class="trace-text trace-raw">${escapeHtml(block.text)}</pre></div>`).join('');
